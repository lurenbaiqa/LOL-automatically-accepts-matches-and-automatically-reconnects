...
class LOL:
    ...
    async def monitor(self):
        while self.running:
            ...
            aaa = await self.get("/lol-gameflow/v1/session")
            bbb = await self.get("/lol-geoinfo/v1/whereami")
            ccc = await self.get("/lol-perks/v1/currentpage")
            print(f"aaa:{aaa}")
            print(f"bbb:{bbb}")
            print(f"ccc:{ccc}")
            zhilianUrl = "https://127.0.0.1:2999/liveclientdata/activeplayer"
            zhilianUrl2 = "https://127.0.0.1:2999/liveclientdata/allgamedata"
            zhilianUrl3 = "https://127.0.0.1:2999/liveclientdata/activeplayername"
            r = requests.get(zhilianUrl, verify=False)
            r2 = requests.get(zhilianUrl2, verify=False)
            r3 = requests.get(zhilianUrl3, verify=False)
            if r.ok:
                lolJson = r.text
                print(f"lolJson:{lolJson}")
                ...
                ddd = str(data["abilities"]["E"]["id"])
                print(ddd)
                r.close()
            if r2.ok:
                lolJson2 = r2.text
                print(f"lolJson2:{lolJson2}")
                r2.close()
            if r3.ok:
                lolJson3 = r3.text
                print(f"lolJson3:{lolJson3}")
                r3.close()
    ...
    async def reconnect(self):
        await self.get("/lol-summoner/v1/reconnect")
        print("已重新连接。")
